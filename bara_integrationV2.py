        ...
        if "promoDateFrom" in item and item["promoDateFrom"] > int(datetime.datetime.now(mexico_city_tz).timestamp() * 1000):
            ...
            if "rsrvDec1" in item:
                is_refresh = refresh_by_price(
                    item["sku"], rsrvDec1=item["rsrvDec1"], pending_promo=True, store_code=store_code)
            elif "saleMode" in item:
                is_refresh = refresh_by_price(
                    item["sku"], saleMode=item["salesMode"], pending_promo=True, store_code=store_code)
            if is_refresh:
                # give me promo's date in dd/mm format
                refresh_date = promo_date.strftime("%d/%m")
            ...
            elif "rsrvDec1" in item_keys:
                # 只可能是 PRM 文件
                is_refresh = refresh_by_price(
                    item["sku"], rsrvDec1=item["rsrvDec1"], store_code=store_code)
            elif "saleMode" in item_keys:
                # 只可能是 RPM 文件
                is_refresh = refresh_by_price(
                    item["sku"], saleMode=item["salesMode"], store_code=store_code)

            if is_refresh:
                # give me today's date in dd/mm format
                today_date = datetime.datetime.now(
                    mexico_city_tz).strftime("%d/%m")
...
def get_allstar_data(store_code, sku, target_data):
    ...
    if response["data"]["pageData"] != []:
        ...
        if target_data in allstar_itemdata:
            return allstar_itemdata[target_data]
        else:
            # sku exists, but target data not found
            return "Target data not found"
    # sku not found
    return "SKU not found"


def refresh_by_price(sku, price1=0, rsrvDec1=0, saleMode="-1", pending_promo=False, store_code="01"):
    allstar_price1 = get_allstar_data(store_code, sku, "price1")
    allstar_rsrvDec1 = get_allstar_data(store_code, sku, "rsrvDec1")
    allstar_saleMode = get_allstar_data(store_code, sku, "saleMode")
    # They can be "sku not found", or "target data not found"

    # if not found in allstar, 100% refresh
    if allstar_price1 == "SKU not found":
        return True

    # determine if the item is in promotion period
    allstar_isPromo = False

    allstar_promoDateTo = get_allstar_data(store_code, sku, "promoDateTo")
    if allstar_promoDateTo != "Target data not found" and allstar_promoDateTo > int(datetime.datetime.now(mexico_city_tz).timestamp() * 1000) and allstar_saleMode != "00":
        allstar_isPromo = True

    # pending promo 的情况看看rsrvDec1是否有变动，如果有变动，那么就刷新。否则不刷新
    """-----------------------------------------------------------------"""
    if pending_promo:
        if allstar_isPromo:
            # 在促销模版中，
            if rsrvDec1 != allstar_rsrvDec1 and rsrvDec1 != 0:
                # rsrvDec1变动，刷新 (saleMode是 00 它刷回正常模版，非 00，它价格变动，所以这种情况不管 saleMode)
                return True
            elif saleMode == "00":
                # rsrvDec1 没变，但是saleMode变成了 00，切回正常模版，刷新
                return True
        else:
            # 在正常模版中
        ...
        if allstar_isPromo:
            ...
            # Q:如果是促销期结束切换成正常模版的那个日期怎么办？rsrvTxt2不会自动防这个日期
            # A:再写一个函数，时不时检测一下，如果是促销期结束，那么就切换成正常模版，rsrvTxt2是促销结束日期
