...
def check_pending_promo_files(customer_code: str, store_code: str, client_id: str, client_secret: str):
    ...
    if os.path.exists(f"current_files/{store_code}/pending_promo/pending_promo.json"):
        with open(f"current_files/{store_code}/pending_promo/pending_promo.json", "r") as f:
            ...
            for promo in pending_promo[:]:
                ...
                # 这里是用来判断 daily_check里加进来的pending 文件,这里是结束 promo 的时间。
                elif "rsrvTxt2" in promo:
                    if promo["rsrvTxt2"] <= int(datetime.datetime.now(mexico_city_tz).timestamp() * 1000):
                        # covert rsrvTxt2 from miliseconds timestamp to dd/mm
                        promo["rsrvTxt2"] = datetime.datetime.fromtimestamp(
                            promo["rsrvTxt2"] / 1000).strftime("%d/%m")
                        pending_items.append([promo])
                        pending_promo.remove(promo)
    ...
    if len(pending_items) > 0:
        print(
            f"Integrating {len(pending_items)} pending files...)\n")
        send_integration(
            customer_code, store_code, client_id, client_secret, pending_items, f"{store_code}/pending_promo.json")
    else:
